...
However, we could not predict anything with this lm (R^2 = 0.14). TODO show this work.
...
# avg likeability score given to you by
...
# merges list of dataframes into a single df
full_const_fit <- lm(dec_o ~ ., data = na.omit(const_attribs))  # TODO find reduced model in git commit history
```
...
TODO: repeat with likability as response var
```{r}
...
# TODO: this is no longer a strictly constants-only model!
full_augmented_fit <- lm(dec_o ~ ., data = na.omit(augmented_attribs))
...
The full model has an adj-R^2 of 0.65 on 76 predictor variables. In order to reduce our model, we ran a stepwise AIC.

TODO: double check that we aren't violating any of the assumptions of stepAIC by looking at diagnostic plots first.
...
Reducing our model based on our stepAIC:
```{r}
...
reduced_augmented_fit <- lm(dec_o ~ attr_o + fun_o + gender + age + field_cd + career_c + 
    tvsports + museums + yoga, data = na.omit(augmented_attribs))

summary(reduced_augmented_fit)
```
Our model stayed with approximately the same adjusted R^2 (0.668), but now we have only 39 predictors. We noticed that career_c and field_c did not improve our R^2 by much, so we ran an F-test
```{r}
anova(reduced_augmented_fit, lm(dec_o ~ attr_o + fun_o + gender + age + 
    tvsports + museums + yoga, data = na.omit(augmented_attribs)) )
```
The F test shows it the model benefits from career_c and field_c, so we will keep them it in model.
...
```{r}
fun_const_fit <- lm(fun_o ~ . -attr_o -sinc_o -intel_o -dec_o -amb_o, data = na.omit(const_attribs))
...
fun_reduced_augmented_fit_2 <- lm(fun_o ~ go_out + exercise + clubbing + gender + field_cd + tv, data = na.omit(smaller_const_attribs))
