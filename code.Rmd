...
const_attribs <- const_attribs[, -which(names(const_attribs) %in% c("tuition", "mn_sat", "income")) ]
# fit a full model
...
```{r}
# since we have a bunch of predictor variables I'm going to run a stepwaise AIC to see if we can find a reduced model
library(MASS)
...
reduced_const_fit <- lm(dec_o ~ attr_o + fun_o + gender + age + field_cd + career_c + 
    tvsports + museums + yoga, data = na.omit(const_attribs))

summary(reduced_const_fit)
```

```{r}
# look at maybe removing career_c and field_c since it didn't improve our R^2 by much. F test shows it helps, so we keep it in model
anova(reduced_const_fit,lm(dec_o ~ attr_o + fun_o + gender + age + 
    tvsports + museums + yoga, data = na.omit(const_attribs)) )
```


```{r}
# check if there are any apparent associations between variables in our model
# dec_o ~ attr_o + fun_o + gender + age + field_cd + career_c + tvsports + museums + yoga
pairs(const_attribs[,c("dec_o", "fun_o", "attr_o", "gender","age", "field_cd", "career_c", "tvsports","museums","yoga")]) 
...
# QQ plot indicates normally distributed errors, and possibly-maybe some heteroskedasticity in our variance.


#next step is to maybe introduce interaction terms?
```

The Q-Q plot tells us that we have normally distributed errors, and the Residual vs Fitted plot tells us that we don't have heterskedasticity in our variance. 
