...
```{r}
library(lmtest)
bptest(dec_o ~ attr_o + fun_o + gender + age + tvsports + museums + yoga, data = na.omit(augmented_attribs))
```

The p value is 0.04484 so there might be some issues with nonconstant error variance. Let's do a transformation with BoxCox.

```{r}
library(MASS)
# had issues with boxcox when y values are zero.
nonzero.augmented_attribs = augmented_attribs[augmented_attribs$dec_o != 0,]
bc <- boxcox(dec_o ~ attr_o + fun_o + gender + age + 
    tvsports + museums + yoga, data = na.omit(nonzero.augmented_attribs))
lambda <- 
```
...
library("corrplot")
reduced_likeability_df <- data.frame(na.omit(augmented_attribs[, c("like_o", "sinc_o", "intel_o", "fun_o", "amb_o",  "age")]))
#corrplot(reduced_likeability_df, method = "color")
cor(reduced_likeability_df)
```
